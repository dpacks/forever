...
async function renderDirectoryListingPage (vault, dirPath, webRoot) {
  // handle the webroot
  webRoot = webRoot || '/'
  ...
  // list files
  var names = []
  try { names = await readdir(vault, realPath(dirPath)) } catch (e) {}

  // stat each file
  var entries = await Promise.all(names.map(async (name) => {
    var entry
    var entryPath = joinPaths(dirPath, name)
    try { entry = await stat(vault, realPath(entryPath)) } catch (e) { return false }
